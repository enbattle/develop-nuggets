## Output Validation & Structured Safety
...
The Instructor library wraps LLM calls with Pydantic validation and automatic retry (introduced in the **LLM Core node**; here we add safety-specific field validators and business-rule constraints):
...
For self-hosted models, grammar-constrained decoding (Outlines library) enforces structure at the token generation level — the model literally cannot produce tokens that violate your schema, eliminating the need for retry logic entirely. See the **LLM Core node** for a full implementation.
...
Over-refusal is also a failure mode. Detect when the model has refused a safe request:
