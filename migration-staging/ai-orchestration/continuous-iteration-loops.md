## Continuous Iteration Loops (Ralph Pattern)
...
The core insight is that **iteration beats perfection on the first try**. Rather than engineering one flawless prompt, you let the agent try, observe what broke, and try again — with the file system or a task list as persistent memory between cycles.
