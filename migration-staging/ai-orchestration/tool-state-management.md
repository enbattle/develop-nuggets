...
Tools and state are the two axes where agent systems break in production. Tool calls are I/O operations with real-world side effects; state is the memory that lets an agent survive failures and resume. Getting both right is what separates a reliable agent from a flaky demo.
