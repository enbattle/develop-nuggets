...
Chain-of-Thought (CoT) prompting is the observation that asking a language model to explain its reasoning before giving an answer dramatically improves accuracy on multi-step problems. It is one of the most impactful prompt engineering techniques discovered to date.
...
For model-level reasoning that goes beyond prompt engineering — extended thinking, process reward models — see the other lessons in this node.
