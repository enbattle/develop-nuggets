...
Chain-of-thought generates a single linear reasoning path. But many problems benefit from exploring multiple approaches, evaluating them, and pursuing the most promising one. Tree of Thoughts (ToT) extends CoT by letting the model generate and evaluate a **tree** of reasoning paths.
...
For tasks where extended thinking is available, Claude's internal reasoning already performs a form of implicit search — evaluating and discarding partial answers internally. This is usually more cost-effective than explicit ToT for well-supported models.
