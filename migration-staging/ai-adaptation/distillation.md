...
Speculative decoding (covered in the LLM Core node) uses a small draft model to propose tokens that a large model verifies. This is a form of distillation at inference time — the draft model's predictions guide the large model's generation, achieving the throughput of the small model with the quality of the large one.
...
A common production pattern: distill a frontier model's task performance into a 7B model, then quantize that model to 4-bit for fast, cheap serving.
