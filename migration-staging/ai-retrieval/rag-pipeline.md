...
RAG runs in two distinct phases: **indexing** (one-time setup) and **querying** (real-time per request).
...
|-----------|-------------|--------------|
| **Chunking** | Splits documents for precise retrieval | Size (512 tokens), overlap (50), strategy |
| **Embeddings** | Converts text to searchable vectors | Model choice (accuracy vs. speed) |
| **Vector DB** | Stores and searches embeddings | Scale: Chroma (local) → Qdrant/Pinecone (prod) |
