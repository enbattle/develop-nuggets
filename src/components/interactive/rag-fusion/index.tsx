...
const INSIGHT =
  'Single-query retrieval is biased toward the exact phrasing used. RAG Fusion treats retrieval as a sampling problem: generate N queries, retrieve N result sets, then let Reciprocal Rank Fusion reward documents that rank well across multiple phrasings. Genuinely relevant documents appear consistently; lucky retrievals from a single phrasing are diluted.';

const TRACE = [
  'Query arrives: "How do transformer attention mechanisms work?" It will not be embedded once.',
  'An LLM generates 3 query variants, each targeting a different slice of the document space (self-attention, QKV matrices, multi-head attention).',
  'Retrieval 1 of 4 — the original query. Top-5 by vector similarity; strong signal on the core attention mechanism.',
  'Retrievals 2-4 run with the variants. Variant 2 surfaces a new "QKV matrix" document; Variant 3 surfaces a new "attention heads" document — both missed by the original query.',
  'Reciprocal Rank Fusion scores each document: RRF(d) = Σ 1 / (k + rank), with k = 60.',
  'Top-5 by RRF score. The QKV and attention-heads documents now rank 3rd and 4th; noise from a single retrieval is suppressed.',
  'The LLM generates from the RRF-fused top-5 — broader and more complete than the original query alone.',
  'Answer returned. ~3x the cost of single-query RAG, but recall improved from ~65% to ~79% on this class of query.',
];

const CHUNKS: TraceChunk[] = [
  { rank: 1, source: 'attention-paper.pdf', score: 0.066, content: 'Transformer architecture uses self-attention... (RRF 0.066 — ranked 1st in 3 of 4 queries)' },
  { rank: 2, source: 'transformers.pdf', score: 0.061, content: 'Multi-head attention projects queries, keys, and values into subspaces... (top-3 in all 4 queries)' },
  { rank: 3, source: 'qkv-matrices.md', score: 0.049, content: 'The query matrix Q, key matrix K, and value matrix V are learned linear projections... (only surfaced by variants)' },
  { rank: 4, source: 'attention-heads.md', score: 0.044, content: 'Each attention head learns to focus on different syntactic and semantic relationships... (consistent across variant retrievals)' },
  { rank: 5, source: 'attention-math.md', score: 0.041, content: 'Scaled dot-product attention: softmax(QKᵀ / √dₖ)V... (mid-rank in 3 of 4 retrievals)' },
];
...
export default function RagFusionInteractive() {
  return (
    <PipelineSummary
      diagramTitle="RAG Fusion pipeline"
      layout={LAYOUT}
      insight={INSIGHT}
      code={CODE}
      trace={TRACE}
