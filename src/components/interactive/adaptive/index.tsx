...
const INSIGHT =
  'Adaptive-RAG optimizes cost and latency by matching retrieval complexity to query complexity. Simple questions skip retrieval entirely using parametric knowledge; only complex ones pay for multi-step iterative retrieval.';

const TRACE = [
  'Query arrives. Adaptive-RAG classifies complexity before deciding a retrieval strategy.',
  'A lightweight classifier analyzes the query: length, question words, named entities, ambiguity signals.',
  'Classification result: MODERATE complexity. The query needs retrieval, but single-step is sufficient.',
  'A single retrieval executes — standard top-K vector search with reranking.',
  'The LLM generates from the single retrieval; context is sufficient for this complexity.',
  'Answer returned. Adaptive routing saved cost by avoiding unnecessary multi-step retrieval.',
];

const CHUNKS: TraceChunk[] = [
  { rank: 1, source: 'adaptive-rag.pdf', score: 0.89, content: 'Adaptive-RAG routes queries to no-retrieval, single-step, or multi-step...' },
  { rank: 2, source: 'query-routing.md', score: 0.84, content: 'Query complexity classification uses features like length and ambiguity...' },
  { rank: 3, source: 'rag-efficiency.pdf', score: 0.78, content: 'Cost optimization through dynamic retrieval strategy selection...' },
];
...
const CODE: CodeTab[] = [
  {
    label: 'Full pipeline',
    lang: 'python',
    source: `# Adaptive-RAG Pipeline with Query Complexity Classification
...
`,
  },
  {
    label: 'Classifier',
    lang: 'python',
    source: `def _classify_complexity(self, query: str) -> QueryComplexity:
    """
...
`,
  },
  {
    ...
    lang: 'python',
    source: `# Adaptive Vector Search Strategy
...
`,
  },
  {
    label: 'Generation',
    lang: 'python',
    source: `# Adaptive Generation with Claude Sonnet 4
...
export default function AdaptiveInteractive() {
  return (
    <PipelineSummary
      diagramTitle="Adaptive-RAG pipeline (moderate-complexity route)"
      layout={LAYOUT}
      insight={INSIGHT}
      code={CODE}
      trace={TRACE}
