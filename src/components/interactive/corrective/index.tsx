...
const STEPS: Step[] = [
  {
    caption: 'Query arrives. CRAG will evaluate retrieval quality before using it.',
    activeNodeIds: ['query'],
    activeEdgeIds: [],
  },
  {
    caption:
      'Initial vector retrieval executes. Results will be evaluated for quality.',
    detail:
      'ml-intro.pdf (0.41), neural-nets.md (0.38), preprocessing.md (0.31) — all broad, none on-topic.',
    activeNodeIds: ['retrieve'],
    activeEdgeIds: ['e1'],
  },
  {
    caption:
      'Evaluator scores each chunk against the query. Scores: 0.41, 0.38, 0.31 — all below the 0.75 threshold.',
    activeNodeIds: ['eval'],
    activeEdgeIds: ['e2'],
  },
  {
    caption:
      'Decision: INCORRECT. All chunks failed quality threshold. The fallback branch activates.',
    activeNodeIds: ['route'],
    activeEdgeIds: ['e3'],
  },
  {
    caption: 'All 3 original chunks are rejected. Web search executes as fallback.',
    activeNodeIds: ['web'],
    activeEdgeIds: ['e4'],
  },
  {
    caption:
      'Web search returns fresh, high-quality results. LLM generates from these instead of the rejected chunks.',
    detail:
      'arxiv.org/crag-paper (0.89), research.ai/eval-rag (0.85), blog.ml/corrective-rag (0.82).',
    activeNodeIds: ['llm'],
    activeEdgeIds: ['e5'],
  },
  {
    caption:
      'Answer returned. Quality gate prevented a low-quality response that Standard RAG would have produced.',
...
export default function CorrectiveInteractive() {
  return (
    <StepThrough
      steps={STEPS}
      diagram={<PipelineFigure layout={LAYOUT} title="Corrective RAG pipeline" />}
      code={CODE}
    />
  );
}
