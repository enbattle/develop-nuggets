...
const STEPS: Step[] = [
  {
    caption:
      'Relational query: "What is the relationship between GPT-4 and Microsoft Copilot?"',
    activeNodeIds: ['query'],
    activeEdgeIds: [],
  },
  {
    caption: 'Entity extraction identifies: OpenAI, GPT-4, Microsoft, Copilot.',
    activeNodeIds: ['extract'],
    activeEdgeIds: ['e1'],
  },
  {
    caption: 'Entry point nodes located in knowledge graph.',
    activeNodeIds: ['graph'],
    activeEdgeIds: ['e2'],
  },
  {
    caption:
      'Hop 1: Direct neighbors retrieved. GPT-4 powers Copilot, Microsoft owns Copilot.',
    activeNodeIds: ['hop1'],
    activeEdgeIds: ['e3'],
  },
  {
    caption:
      'Hop 2: Indirect relationships. Microsoft invested in OpenAI, OpenAI licensed to Microsoft.',
    activeNodeIds: ['hop2'],
    activeEdgeIds: ['e4'],
  },
  {
    caption:
      'Generation with full relational context from graph traversal.',
    activeNodeIds: ['llm'],
    activeEdgeIds: ['e5'],
  },
  {
    caption:
      'Answer describes investment, licensing, and product dependencies.',
...
export default function GraphInteractive() {
  return (
    <div className="flex flex-col gap-6">
      <StepThrough
        steps={STEPS}
        diagram={<PipelineFigure layout={LAYOUT} title="GraphRAG pipeline" />}
        code={CODE}
      />
      <KnowledgeGraph
        nodes={GRAPH_NODES}
        edges={GRAPH_EDGES}
        caption="the demo corpus graph traversed above"
      />
    </div>
  );
}
